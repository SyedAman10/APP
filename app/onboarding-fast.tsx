...
const fastTrackQuestions = [
  {
    ...
    key: 'inspirationFigure' as keyof FastTrackOnboardingData,
  },
  {
    id: 'inspirationQuality',
    title: 'What is it about them — their voice, wisdom, humor, or presence — that makes you choose them?',
    placeholder: 'e.g., their gentle wisdom, their humor, their courage, their perspective...',
    key: 'inspirationQuality' as keyof FastTrackOnboardingData,
  },
  {
    id: 'actorAuthorStyle',
    title: 'Who\'s one actor or author whose style or way of being really resonates with you?',
    placeholder: 'e.g., Maya Angelou, Morgan Freeman, Brené Brown, David Attenborough...',
    key: 'actorAuthorStyle' as keyof FastTrackOnboardingData,
  },
  {
    id: 'actorAuthorTone',
    title: 'What part of their style or tone feels most supportive to you?',
    placeholder: 'e.g., their calm presence, their passionate delivery, their gentle humor...',
    key: 'actorAuthorTone' as keyof FastTrackOnboardingData,
  },
  {
    id: 'spiritualPractices',
    title: 'Are there any traditions, practices, or things that matter most to you in life?',
    placeholder: 'e.g., meditation, nature, family, creativity, service to others...',
    key: 'spiritualPractices' as keyof FastTrackOnboardingData,
  },
  {
    id: 'spiritualReflection',
    title: 'How would you want your companion to reflect or honor that?',
    placeholder: 'e.g., remind me of what matters, help me stay grounded, encourage my growth...',
    key: 'spiritualReflection' as keyof FastTrackOnboardingData,
  },
  {
    id: 'chapterTitle',
    ...
    key: 'chapterTitle' as keyof FastTrackOnboardingData,
  },
  {
    id: 'primaryHope',
    title: 'What\'s the single biggest change or shift you hope this journey will bring?',
    placeholder: 'e.g., more confidence, better relationships, inner peace, clarity about my purpose...',
    key: 'primaryHope' as keyof FastTrackOnboardingData,
  },
];

export default function FastTrackOnboardingScreen() {
  const { setOnboardingData } = useOnboarding();
  const { saveOnboardingCompletion } = useOnboardingSync();
  const [currentStep, setCurrentStep] = useState(0);
  const [showProfileCompletion, setShowProfileCompletion] = useState(false);
  const [onboardingData, setOnboardingDataLocal] = useState<FastTrackOnboardingData>({
    inspirationFigure: '',
    inspirationQuality: '',
    actorAuthorStyle: '',
    actorAuthorTone: '',
  ...
  const currentQuestion = fastTrackQuestions[currentStep];
  const isLastStep = currentStep === fastTrackQuestions.length - 1;
  ...
  const handleNext = async () => {
    if (isLastStep) {
      ...
      try {
        console.log('💾 Completing fast-track onboarding...');
        
        const response = await profileAPI.completeOnboarding(onboardingRequest);
        
        if (response.success) {
          console.log('✅ Fast-track onboarding completed successfully:', response.data);
            ...
            <View style={styles.progressContainer}>
              <View style={styles.progressBar}>
                <View 
                  style={[
                    styles.progressFill, 
                    { width: `${((currentStep + 1) / fastTrackQuestions.length) * 100}%` }
                  ]} 
                />
              </View>
              <Text style={styles.progressText}>
                {currentStep + 1} of {fastTrackQuestions.length}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.privacyNote}>
            <BlurView intensity={10} tint="dark" style={styles.privacyBlur}>
              <Text style={styles.privacyText}>
                🔒 Your responses are encrypted and stored locally. You can always complete the full experience later for deeper personalization.
              </Text>
            </BlurView>
          </View>
        </ScrollView>
